...
# Start the Electron
if os.name == 'nt':
    print(f"Starting electron at {os.path.join(TORRENT_LAN_DIR,'my-app','client','dist','win-unpacked','my-app.exe')}")
    electron_executable = os.path.join(TORRENT_LAN_DIR, 'my-app','client', 'dist', 'win-unpacked', 'my-app.exe')
    frontend_process = subprocess.Popen([electron_executable], cwd=TORRENT_LAN_DIR)
elif os.name=='posix':
    print(f"Starting electron at {os.path.join(TORRENT_LAN_DIR,'my-app','client','dist','linux-unpacked','my-app')}")
    electron_executable = os.path.join(TORRENT_LAN_DIR, 'my-app','client', 'dist', 'linux-unpacked', 'my-app')
    frontend_process = subprocess.Popen([electron_executable], cwd=TORRENT_LAN_DIR)
elif os.name=='mac':
    print(f"Starting electron at {os.path.join(TORRENT_LAN_DIR,'my-app','client','dist','mac-unpacked','my-app.app')}")
    electron_executable = os.path.join(TORRENT_LAN_DIR, 'my-app','client', 'dist', 'mac-unpacked', 'my-app.app')
    frontend_process = subprocess.Popen([electron_executable], cwd=TORRENT_LAN_DIR)
